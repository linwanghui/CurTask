...
import { _decorator, Asset, Prefab, Node, AssetManager, error, assetManager, resources, instantiate } from 'cc';
interface CacheData {
    asset: Asset;
    release: boolean;
    lastLoadTime: number;
...
export default class Res {
    ...
    /** 对资源有引用的对象实例与资源路径 */
    private static _objPath: Map<Object, string> = new Map();
    /** 资源缓存 */
    private static _assetCache: Map<typeof Asset, Map<string, CacheData>> = new Map();
    /** 对象池 */
     ...
     * 获取新的CacheData
     */
    private static getCacheData(asset: Asset, release: boolean): CacheData {
     ...
     */
    private static cacheAsset(url: string, asset: Asset, type: typeof Asset, release: boolean = true): void {
        if (!asset) {
            return;
        }
     ...
     * 记录对资源有引用的对象
     */
    private static writeObject(url: string, type: typeof Asset, obj: Object): void {
        let cacheMap: Map<string, CacheData> = this._assetCache.get(type);
        if (!cacheMap) {
            error(`[Res.writeObject] cacheMap is null, url: ${url}`);
            return;
        }
        ...
        if (!cacheData) {
            error(`[Res.writeObject] cacheData is null, url: ${url}`);
            return;
        }
     ...
     * @param type 资源类型
     */
    public static get<T extends Asset>(url: string, type: typeof Asset): T | null {
     ...
     */
    public static loadBundle(nameOrUrl: string): Promise<AssetManager.Bundle> {
        return new Promise((resolve, reject) => {
            assetManager.loadBundle(nameOrUrl, (Error: Error, bundle: AssetManager.Bundle) => {
                if (Error) {
                    error(`[Res.loadBundle] error: ${Error}`);
                    resolve(null);
                } else {
                    resolve(bundle);
                }
     ...
     * @param release 资源是否需要释放
     */
    public static async load<T extends Asset>(url: string, type: typeof Asset, release: boolean = true): Promise<T | null> {
        if (!url) {
            error(`[Res.load] url is empty`);
            return null;
        }
        ...
        if (parseData.bundle && !assetManager.getBundle(parseData.bundle)) {
            await this.loadBundle(parseData.bundle);
        }

        asset = await new Promise((resolve, reject) => {
            let bundle: AssetManager.Bundle = parseData.bundle ? assetManager.getBundle(parseData.bundle) : resources;
            if (!bundle) {
                error(`[Res.load] cant find bundle: ${url}`);
                resolve(null);
                return;
            }
            ...
            bundle.load(parseData.loadUrl, type, (Error: Error, resource: T) => {
                if (Error) {
                    error(`[Res.load] load error: ${Error}`);
                    resolve(null);
                } else {
     ...
     * @param release 资源是否需要释放
     */
    public static async loadDir<T extends Asset>(url: string, type: typeof Asset, release: boolean = true): Promise<T[]> {
        if (!url) {
            error(`[Res.load] url is empty`);
            return [];
        }
        ...
        return new Promise((resolve, reject) => {
            let bundle: AssetManager.Bundle = parseData.bundle ? assetManager.getBundle(parseData.bundle) : resources;
            if (!bundle) {
                error(`[Res.loadDir] cant find bundle: ${url}`);
                resolve([]);
                return;
            }
            ...
            bundle.loadDir(parseData.loadUrl, type, (Error: Error, resource: T[]) => {
                if (Error) {
                    error(`[Res.loadDir] load error: ${Error}`);
                    resolve([]);
                } else {
    ...
    public static instantiate(original: Node | Prefab, related?: Node | Prefab): Node {
        ...
        if (url) {
            this.writeObject(url, Prefab, node);
        }
        return node;
    }
    /**
    ...
    public static getNativeUrlByResources(url: string, ext: string, isNative: boolean = true): string {
        try {
            let nativeUrl = assetManager["_transform"]({ path: url, bundle: AssetManager.BuiltinBundleName.RESOURCES, __isNative__: isNative, ext: ext });
            return nativeUrl;
        } catch (er) {
            error(`[Res.getNativeUrlByResources] error url: ${url}`);
            return "";
        }
    }
}