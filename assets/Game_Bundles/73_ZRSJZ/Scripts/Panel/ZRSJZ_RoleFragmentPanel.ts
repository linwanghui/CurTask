...
@ccclass('ZRSJZ_RoleFragmentPanel')
export class ZRSJZ_RoleFragmentPanel extends ZRSJZ_Panel {
    ...
    public OnButtonClick(event: EventTouch): void {
        ...
        this._lastClick = now;
        const reward = ZRSJZ_FragmentService.CreateVideoReward(ZRSJZ_FragmentService.RewardCount, '英雄碎片');
        Banner.Instance.ShowVideoAd(() => {
            const error = reward();
            ZRSJZ_UIManager.Instance?.ShowTip(error || `获得${ZRSJZ_FragmentService.RewardCount}个英雄碎片`);
