...
export const ZRSJZ_ENHANCEMENT_STATS: Record<EnhancementStat, { Icon: string; Suffix: string; Description: string }> = {
    '攻击': { Icon: '攻击', Suffix: '', Description: '增加玩家枪械和近战攻击的伤害。' },
    '生命': { Icon: '生命', Suffix: '', Description: '增加玩家生命上限，进入战斗时生效。' },
    '移速': { Icon: '移速', Suffix: '%', Description: '提高玩家移动速度。' },
    '防御': { Icon: '防御', Suffix: '', Description: '在护甲减伤后抵挡固定伤害；非零攻击至少造成1点伤害。' },
...
const normal: [EnhancementStat, number][] = [
    ['技能伤害', 3], ['技能冷却', 2], ['生命', 20], ['攻击', 10], ['防御', 5],
    ['移速', 2], ['生命', 20], ['攻击', 10], ['技能伤害', 3], ['防御', 5],
];
const materials = [
    ['切割刀', '工业图纸'], ['黑色手表', '量子U盘'], ['哑铃', '沙袋'],
    ['高精数显卡尺', '电动马达'], ['太阳能板', '脑机数据'],
    ['协议箱', '汽车燃油'], ['信息终端', '除颤器'], ['高速阵列', '供能单元'],
    ['军用电台', '呼吸机'], ['反应炉', '医疗机器人'],
];
export const ZRSJZ_ENHANCEMENT_NODES: readonly EnhancementNode[] = Array.from(
    { length: ZRSJZ_ENHANCEMENT_MAX_LEVEL }, (_, i): EnhancementNode[] => {
        ...
        const cost = (special: boolean): Pick<EnhancementNode, 'Gold' | 'Materials'> => ({
            ...
            Materials: materials[tier].map(PropName => ({ PropName, Count: 1 + Math.floor(tier / 3) + (special ? 1 : 0) })),
        });
