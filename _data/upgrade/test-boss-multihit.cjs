...
const ctx={ZRSJZ_FriendlyDamageService:damage,Coop:{SendArea(){}}};
for(const [i,[name,config]]of [...c.ZRSJZ_BOSS_CONFIG].entries()){
 ...
 const boss={node:{worldPosition:{x:0,y:0}},DamageMultiplier:1,RefreshAttackDirection(){},_activeAttackHitCount:0,_activeAttackEventTimes:new Set()};
 ...
 for(const skill of [config.NormalAttack,...config.Skills]){
  ...
  for(const moving of [false,true]){
   ...
   events.forEach((event,index)=>{player.node.worldPosition.x=moving&&index%2?skill.DamageRange+100:skill.DamageRange-1;attack.call(boss,event.name,event.time);attack.call(boss,event.name,event.time)});
