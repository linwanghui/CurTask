const ts=require('../../extensions/cocos-mcp-server-main/node_modules/typescript'),fs=require('fs'),path=require('path');
...
const files=JSON.parse(fs.readFileSync(path.join(__dirname,'fragment-changed.json'))).map(p=>path.join(root,p));
